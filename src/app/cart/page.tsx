...
const CartPage = () => {
  ...
  const handleCheckout = async () => {
    if (!wixConfigured) {
      alert(
        "Checkout functionality is disabled in demo mode. Please configure your Wix environment variables.",
      );
      return;
    }
    ...
    <div className="min-h-[calc(100vh-80px)] px-4 md:px-8 lg:px-16 xl:px-32 2xl:px-64 py-16">
      ...
      {!wixConfigured ? (
        <div className="text-center py-12 bg-amber-50 rounded-lg border border-amber-200">
          <p className="text-xl font-semibold text-amber-800 mb-2">
            Cart Disabled (Demo Mode)
          </p>
          <p className="text-gray-700">
            Configure your Wix environment variables to enable cart and checkout
            functionality.
          </p>
        </div>
      ) : isLoading ? (
                ...
                <div className="flex-1">
                  ...
                  {item.descriptionLines?.map((desc, index) => (
                    <p key={index} className="text-sm text-gray-600">
                      {String(desc.name?.original || desc.name || "")}:{" "}
